    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST' && KecsoRequest::CouriorsInsert()) {
        ...
        if (empty($error)) {
            try {
                ...
                $_SESSION['success_message'] = 'A futár sikeresen hozzáadva.';
                header("Location: " . Config::TATAB_URL_COURIORDATA);
                exit();
            } catch (Exception $e) {
...
public function courioraddress2($param): string
{
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['newAddress'])) {
        ...
        if (!ctype_digit($_POST['ids'])) {
            ...
            header("Location: " . Config::TATAB_URL_COURIORADDRESS);
            exit();
        }
