...
private static function DisplayCars()
 {
    ...
    if (!empty($cars)) {
        ...
        $html .= '<a href="'.Config::NYERGES_URL_CARDATA.'">Gépjárművek adatai</a>';
        $html .= '<a href="'.Config::NYERGES_URL_CARCOST.'">Javítási költségek</a>';
        $html .= '</div>';
    } else {
...
//Carcost aloldal 
  public static function CarCost($carcost, $editcarcost = null)
    ...
    private static function DisplayCarCostGroup($ids, $items, $editcarcost)
    {
        ...
        foreach ($items as $item) {
            $html .= '<tr>
                        ...
                        <td>
                            <form method="post" action="' . Config::NYERGES_URL_CARCOST . '?operation=carcost4&param=' . htmlspecialchars($item['_id'] ?? '') . '" style="display:inline;">
                             <input type="hidden" name="updateCarCostId" value="' . ($item['_id'] ?? '') . '">
                             <button type="submit" name="updateCarcost">Szerkesztés</button>
                            </form>
                                <form method="post" action="' . Config::NYERGES_URL_CARCOST . '" style="display:inline;">
    ...
    {
        $html = '<form method="post" action="' . Config::NYERGES_URL_CARCOST . '">';
        $html .= IndexView::CreateInput('Rendszám', 'ids');
    ...
    private static function CarCostEdit($editcarcost)
   {
    ...
    /*public static function ShowCostByGroup($cars, $startDate, $endDate)
    {
        ...
        $html .= '<form method="post" action="' . htmlspecialchars(Config::NYERGES_URL_CARCOST) . '">';
