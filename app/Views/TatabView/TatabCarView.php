...
private static function DisplayCars()
 {
    ...
    if (!empty($cars)) {
        ...
        $html .= '<a href="'.Config::TATAB_URL_CARDATA.'">Gépjárművek adatai</a>';
        $html .= '<a href="'.Config::TATAB_URL_CARCOST.'">Javítási költségek</a>';
        $html .= '</div>';
    } else {
...
private static function DisplayCarDataGroup($ids, $items, $editcardata)
{
    ...
    foreach ($items as $item) {
        $html .= '<tr>
                    ...
                    <td>
                        <form method="post" action="' . Config::TATAB_URL_CARDATA . '?operation=cardata2&param=' . htmlspecialchars($item['_id'] ?? '') . '" style="display:inline;">
                            <input type="hidden" name="updateCarDataId" value="' . ($item['_id'] ?? '') . '">
                            <button type="submit" name="updateCarData">Szerkesztés</button>
                        </form>
                        <form method="post" action="' . Config::TATAB_URL_CARDATA . '" style="display:inline;">
...
{
    $html = '<form method="post" action="' . Config::TATAB_URL_CARDATA . '">';
    $html .= IndexView::CreateInput('Rendszám', 'ids');
...
//Carcost aloldal 
  public static function CarCost($carcost, $editcarcost = null)
    ...
    private static function DisplayCarCostGroup($ids, $items, $editcarcost)
    {
        ...
        foreach ($items as $item) {
            $html .= '<tr>
                        ...
                        <td>
                            <form method="post" action="' . Config::TATAB_URL_CARCOST . '?operation=carcost2&param=' . htmlspecialchars($item['_id'] ?? '') . '" style="display:inline;">
                             <input type="hidden" name="updateCarCostId" value="' . ($item['_id'] ?? '') . '">
                             <button type="submit" name="updateCarcost">Szerkesztés</button>
                            </form>
                                <form method="post" action="' . Config::TATAB_URL_CARCOST . '" style="display:inline;">
    ...
    {
        $html = '<form method="post" action="' . Config::TATAB_URL_CARCOST . '">';
        $html .= IndexView::CreateInput('Rendszám', 'ids');
        $html .= '<label for="date">Időpont:</label>';
        $html .= '<input type="datetime-local" id="date" name="date">';
   ...
   /*public static function ShowCostByGroup($cars, $startDate, $endDate)
   {
       ...
       $html .= '<form method="post" action="' . htmlspecialchars(Config::TATAB_URL_CARCOST) . '">';
