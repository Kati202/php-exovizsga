...
public static function ShowCourior()
   ...
   public static function CouriorData($couriordata, $editcourior = null, $id = null)
   {
    ...
    $html .= '<form method="post" action="' . Config::TATAB_URL_COURIORDATA . '?param=' . $id . '">';
    $html .= IndexView::CreateInput('Azonosító', 'ids');
    ...
    public static function CouriorEdit($courior)
{
    ...
    $html = '<form method="post" action="' . Config::TATAB_URL_COURIORDATA . '">';
...
private static function DisplayCouriorGroup($couriors, $editcourior = null)
{
    ...
    foreach ($couriors as $courior) {
        ...
        $html .= '<tr>
                    ...
                    <td>
                        <form method="post" action="' . Config::TATAB_URL_COURIORDATA . '?param=' . $couriorId . '" style="display:inline;">
...
private static function DisplayCouriors()
{
    ...
    if (!empty($couriors)) {
        foreach ($couriors as $courior) {
            $html .= '<tr>
                        ...
                        <td>' . htmlspecialchars($courior['name'] ?? '') . '</td>
                        <td>
                            <form method="post" action="' . Config::TATAB_URL . '" style="display:inline;">
        ...
        if (isset($courior)) {
            ...
            $html .= '<a href="' . Config::TATAB_URL_COURIORDATA  .'">Személyes adatok</a>
                      <a href="' . Config::TATAB_URL_COURIORADDRESS .'">Hónapos szétbontás</a>';
           $html .= '<div>';
        }
    } else {
    ...
    private static function DisplayAddressGroup($ids, $items, $editaddress)
    {
        ...
        foreach ($items as $item) {
            $html .= '<tr>
                        ...
                        <td>
                            <form method="post" action="' . Config::TATAB_URL_COURIORADDRESS . '?operation=courioraddress2&param=' . htmlspecialchars($item['_id'] ?? '') . '" style="display:inline;">
                             <input type="hidden" name="updateAddressId" value="' . ($item['_id'] ?? '') . '">
                             <button type="submit" name="updateAddress">Szerkesztés</button>
                            </form>
                                <form method="post" action="' . Config::TATAB_URL_COURIORADDRESS . '" style="display:inline;">
    ...
    {
        $html = '<form method="post" action="' . Config::TATAB_URL_COURIORADDRESS . '">';
    ...
    public static function ShowDeliveriesByGroup($deliveries, $startDate, $endDate, $ids, $selectedIds = [])
    {
        ...
        $html .= '<form method="post" action="' . htmlspecialchars(Config::TATAB_URL_COURIORADDRESS) . '">
